...
const MAX_CONSECUTIVE_MISSES = 20; // legacy guard used elsewhere
const MISS_LIMIT = Number(process.env.PPUSA_CONSECUTIVE_MISS_LIMIT || '100');
...
// Start scanning from the very first user by default; override via PPUSA_START_USER_ID
const DEFAULT_START_ID = Number(process.env.PPUSA_START_USER_ID || '1');
// Allow raising/lowering the cap of new profiles discovered per run
    ...
    try {
      ...
      try {
        ...
        if (existingTargetIds.length) {
          ...
          const BATCH_SIZE = 15;
          // Keep batches modest; we will await the whole batch to ensure completion
          ...
          for (let i = 0; i < existingTargetIds.length; i += BATCH_SIZE) {
            ...
            // Await the whole batch to ensure all profiles in the batch are processed
            await Promise.all(batchPromises);
          }
        }

        // Look for new profiles with parallel processing
        let consecutiveMisses = 0; // legacy per-result metric (kept)
        let consecutiveMissIds = 0; // strict per-ID miss counter
        let scannedIds = 0;
        const rangeStart = id;
        let stopReason = null;
        if (effectiveNewStartId > 0) {
          ...
          const maxNewProfiles = Math.max(1, MAX_NEW_PROFILES);
          ...
          while (true) {
            ...
            // Process full batch (we control size); map results to ids
            ...
            for (let cur = id; cur < batchEnd; cur++) {
              ...
              if (newProfilesFound >= maxNewProfiles) {
                stopReason = `max_new_profiles (${maxNewProfiles})`;
                shouldStop = true; break;
              }
              if (consecutiveMissIds >= MISS_LIMIT) {
                stopReason = `consecutive_misses (${MISS_LIMIT})`;
                shouldStop = true; break;
              }
              if (scannedIds >= MAX_IDS_PER_RUN) {
                stopReason = `max_ids_per_run (${MAX_IDS_PER_RUN})`;
                shouldStop = true; break;
              }
            }
            ...
            if (batchFound === 0 && consecutiveMisses >= MAX_CONSECUTIVE_MISSES) {
              stopReason = stopReason || `legacy_consecutive_misses (${MAX_CONSECUTIVE_MISSES})`;
              break;
            }
            if (shouldStop) break;
          }
          ...
          if (scannedIds > 0) {
            const msg = `Discovery summary: rangeStart=${rangeStart}, lastTriedId=${id - 1}, scanned=${scannedIds}, newFound=${newProfilesFound}, stopReason=${stopReason || 'loop_end'}`;
            console.log(msg);
            await this.logToChannel(msg);
          }
        }
