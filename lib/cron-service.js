...
class CronService {
  ...
  async runHourlyUpdate() {
    ...
    try {
      ...
      // Run all updates in sequence with individual error handling
      try {
      ...
      try {
        ...
        await this.logToChannel(`❌ Primary update failed: ${err?.message || String(err)}`, true);
      }
      ...
      try {
        ...
        await this.logToChannel(`❌ Race update failed: ${err?.message || String(err)}`, true);
      }
