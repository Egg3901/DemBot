...
const MAX_UPDATE_TIME = 10 * 60 * 1000; // 10 minutes max per update cycle
...
class CronService {
      ...
      try {
        ...
        for (const state of statesList) {
          try {
            ...
            await page.goto(stateUrl, { waitUntil: 'domcontentloaded', timeout: 8000 }).catch(() => {});
      ...
      try {
        ...
        for (const state of statesList) {
          try {
            const stateId = resolveStateIdFromIndex(statesIndexHtml, state.name);
            if (!stateId) continue;
            await page.goto(`${BASE}/states/${stateId}/primaries`, { waitUntil: 'domcontentloaded', timeout: 8000 }).catch(() => {});
            ...
            for (const race of races) {
              ...
              for (const p of ['dem', 'gop']) {
                ...
                await page.goto(m.url, { waitUntil: 'domcontentloaded', timeout: 8000 }).catch(() => {});
      ...
      try {
        ...
        for (const state of statesList) {
          try {
            const stateId = resolveStateIdFromIndex(statesIndexHtml, state.name);
            if (!stateId) continue;
            await page.goto(`${BASE}/states/${stateId}/elections`, { waitUntil: 'domcontentloaded', timeout: 8000 }).catch(() => {});
            const electionsHtml = await page.content();

            for (const race of races) {
              const meta = findRaceFromElections(electionsHtml, race.label);
              if (!meta?.url) continue;
              await page.goto(meta.url, { waitUntil: 'domcontentloaded', timeout: 8000 }).catch(() => {});
  ...
  async runHourlyUpdate() {
    ...
    const timeoutHandle = setTimeout(async () => {
      if (this.isRunning) {
        console.error('🚨 CRON UPDATE TIMEOUT - Force stopping after 10 minutes');
        this.isRunning = false;
        await this.logToChannel('🚨 UPDATE TIMEOUT: Process exceeded 10 minutes, force stopped', true);
      }
    }, MAX_UPDATE_TIME);

    try {
      await this.logToChannel('🔄 Starting hourly automated updates (PARALLEL)...');

      // Run all updates in PARALLEL for maximum performance
      const updatePromises = [
        this.updateProfiles().catch(err => {
          console.error('Profile update failed:', err);
          this.logToChannel(`❌ Profile update failed: ${err?.message || String(err)}`, true);
          return null;
        }),
        this.updateStates().catch(err => {
          console.error('State update failed:', err);
          this.logToChannel(`❌ State update failed: ${err?.message || String(err)}`, true);
          return null;
        }),
        this.updatePrimaries().catch(err => {
          console.error('Primary update failed:', err);
          this.logToChannel(`❌ Primary update failed: ${err?.message || String(err)}`, true);
          return null;
        }),
        this.updateRaces().catch(err => {
