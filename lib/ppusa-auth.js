...
async function authenticateAndNavigate({ url, waitUntil = 'domcontentloaded', debug = config.debug } = {}) {
  // Allow cookie-only auth: only require credentials if no cookie provided
  const hasCookie = !!(config.cookie && String(config.cookie).trim());
  if (!hasCookie && (!config.email || !config.password)) {
    const err = new PPUSAAuthError('Missing PPUSA_EMAIL or PPUSA_PASSWORD (or set PPUSA_COOKIE)', { email: !!config.email, password: !!config.password });
    markPPUSAAuthError(err);
    throw err;
  }
  ...
  while (retryCount <= maxRetries) {
    try {
      ...
      page = await newPage({ intercept: true });
  ...
  // Headers/UA/timezone already configured by browser-manager newPage
  ...
  try {
    ...
    // Fast path 1: Already logged in (persistent profile)
    try {
      await page.goto(targetUrl, { waitUntil, timeout: config.navTimeout });
      const onLogin = /\/login\b/i.test(page.url());
      pushAction(actions, 'fastpath-target', !onLogin, { finalUrl: page.url() });
      if (!onLogin) {
      ...
      pushAction(actions, 'fastpath-target', false, { error: fpErr?.message || String(fpErr) });
    }

    // Fast path 2: Cookie-based auth if provided
    try {
      const ok = await tryCookieAuth(page, targetUrl, actions);
      if (ok) {
        markPPUSAAuthSuccess();
        const html = await page.content();
        return { browser, page, html, finalUrl: page.url(), status: 200, actions };
      ...
      pushAction(actions, 'cookie-auth', false, { error: cookieErr?.message || String(cookieErr) });
    }
