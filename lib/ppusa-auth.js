...
async function authenticateAndNavigate({ url, waitUntil = 'domcontentloaded', debug = config.debug } = {}) {
  ...
  try {
    ...
    if (submitSel) {
      ...
      const fallbackSubmit = await page.evaluate(() => {
        const buttons = Array.from(document.querySelectorAll('button, input[type="submit"]'));
        const textButtons = buttons.filter(el =>
          el.textContent &&
          /\b(?:sign in|log in|login|submit)\b/i.test(el.textContent.trim())
        );
        return textButtons.length > 0 ? textButtons[0] : null;
      });

      if (fallbackSubmit) {
        pushAction(actions, 'login-submit-fallback', true, { foundByText: true });
        await Promise.all([
          page.waitForNavigation({ waitUntil, timeout: config.navTimeout }).catch(() => null),
          page.click(fallbackSubmit),
        ]);
      } else {
        pushAction(actions, 'login-submit-fallback', false, { reason: 'no text buttons found' });
        await Promise.all([
          page.waitForNavigation({ waitUntil, timeout: config.navTimeout }).catch(() => null),
    ...
    if (browser) {
      try {
        ...
        console.warn('⚠️ Failed to close browser properly:', closeErr.message);
