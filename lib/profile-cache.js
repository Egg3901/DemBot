...
function isValidDiscord(discord) {
  if (!discord || typeof discord !== 'string') return false;
  if (discord === 'null' || discord === 'undefined' || discord === 'None') return false;
  if (discord.trim().length === 0) return false;
  return true;
}
...
function loadProfileDb() {
  ...
  const finalDb = ensureDbShape(db);
  
  // Rebuild byDiscord index if missing or incomplete
  if (Object.keys(finalDb.byDiscord).length < Object.keys(finalDb.profiles).length * 0.5) {
    finalDb.byDiscord = {};
    for (const [id, profile] of Object.entries(finalDb.profiles)) {
      if (isValidDiscord(profile.discord)) {
        addDiscordIndex(finalDb, profile.discord, id);
      }
    }
  }
  
  return { db: finalDb, jsonPath: JSON_PATH };
}
...
function stateToRegion(stateName) {
  ...
  const abbr = clean.length === 2 ? clean.toUpperCase() : null;
  if (abbr) {
    if (abbr === 'TX') return 'south';
    if (abbr === 'CA') return 'west';
    if (abbr === 'NY') return 'northeast';
    if (abbr === 'FL') return 'south';
    if (abbr === 'IL') return 'rust_belt';
  }
...
function buildProfileRecord(id, info) {
  ...
  return {
    id: recordId,
    name: info?.name || null,
    discord: isValidDiscord(info?.discord) ? info.discord : null,
    ...
    // Persist latest detected policy positions (from "Political Info" table)
    policyPositions: info?.policyPositions || null,
    es: info?.es || null,
...
function mergeProfileRecord(db, id, info) {
  ...
  if (isValidDiscord(record.discord)) addDiscordIndex(target, record.discord, numericId);
  return record;
}

function cleanupDatabase() {
  const { db } = loadProfileDb();
  const cleaned = { profiles: {}, byDiscord: {} };
  const seenNames = new Set();
  let duplicatesRemoved = 0;
  
  // Remove duplicates and invalid profiles
  for (const [id, profile] of Object.entries(db.profiles || {})) {
    ...
    seenNames.add(name);
    cleaned.profiles[id] = profile;
    
    // Rebuild byDiscord index
    if (isValidDiscord(profile.discord)) {
      addDiscordIndex(cleaned, profile.discord, id);
    }
  }
  
  if (duplicatesRemoved > 0) {
    console.log(`[Database Cleanup] Removed ${duplicatesRemoved} duplicate profiles`);
    writeProfileDb(cleaned);
  }
