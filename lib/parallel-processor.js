...
class ParallelProcessor {
  constructor(options = {}) {
    this.maxConcurrency = options.maxConcurrency || 15; // Increased from 5 to 15
    this.batchSize = options.batchSize || 20; // Increased from 10 to 20
    this.delayBetweenBatches = options.delayBetweenBatches || 500; // Reduced from 1000ms to 500ms
    this.retryAttempts = options.retryAttempts || 1; // Reduced from 2 to 1
    this.retryDelay = options.retryDelay || 1000; // Reduced from 2000ms to 1000ms
  }
  ...
  async processProfiles(profileIds, profileProcessor, options = {}) {
    const {
      onProgress = null,
      onError = null,
      maxConcurrency = 12, // Increased from 3 to 12 for profile scraping
      batchSize = 15 // Increased from 5 to 15
    } = options;

    return this.processInParallel(profileIds, profileProcessor, {
      maxConcurrency,
      batchSize,
      delayBetweenBatches: 300, // Reduced from 2000ms to 300ms
      onProgress,
      onError
    });
  }
  ...
  async processRaces(raceData, raceProcessor, options = {}) {
    const {
      onProgress = null,
      onError = null,
      maxConcurrency = 8, // Increased from 4 to 8
      batchSize = 12 // Increased from 8 to 12
    } = options;

    return this.processInParallel(raceData, raceProcessor, {
      maxConcurrency,
      batchSize,
      delayBetweenBatches: 200, // Reduced from 1500ms to 200ms
      onProgress,
      onError
    });
  }
