...
const DEBUG = String(E('PPUSA_DEBUG', 'false')).toLowerCase() === 'true';
const LOGIN_UA = E(
  'PPUSA_LOGIN_UA',
...
async function loginAndGet(url) {
  ...
  if (typeof page.setUserAgent === 'function') {
    await page.setUserAgent(LOGIN_UA);
  }
  try {
    const targetUrl = url.startsWith('http') ? url : BASE + url;

    // Try session cookie if provided (fast-path bypass)
    const cookieStr = E('PPUSA_COOKIE', '');
    if (cookieStr) {
      try {
        const u = new URL(BASE);
        const firstKV = cookieStr.split(';')[0];
        const [name, ...rest] = firstKV.split('=');
        const value = rest.join('=');
        if (name && value) {
          await page.setCookie({ name: name.trim(), value: value.trim(), domain: u.hostname, path: '/' });
          await page.goto(targetUrl, { waitUntil: 'networkidle2' });
          if (!/\/login\b/i.test(page.url())) {
            const resp = null; // no direct response handle in this branch
            const html = await page.content();
            return { browser, page, html, status: 200, finalUrl: page.url() };
          }
        }
      } catch (_) {
    ...
    // basic selectors (expanded for resilience)
    ...
    if (/\/login\b/i.test(page.url())) {
      if (DEBUG && page.screenshot) {
        try { await page.screenshot({ path: 'ppusa_login_rejected.png' }); } catch (_) {}
      }
