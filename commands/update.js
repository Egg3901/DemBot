...
module.exports = {
  ...
  async execute(interaction) {
    ...
    try {
      ...
      // Soft auth check: if homepage looks logged out, warn but continue
      try {
        ...
        const looksLogin = /\bLogin\b/i.test(pageTitle);
        if (looksLogin && !hasProfilePic) {
          await interaction.editReply('Warning: homepage appears logged out, proceeding with scanning anyway...');
        }
      } catch (_) {}
