...
async function performWebSend({ type, name, amount, debug }) {
  ...
  try {
    ...
    try {
      ...
      // brief wait to allow suggestions to populate
      ...
      for (const sel of SUGGESTION_SELECTORS) {
        ...
        // Try to click a suggestion that contains the typed name, else first item
    ...
    // Submit form
    const formSubmitSel = '#partyTransactions button[type="submit"]';
    if (!(await page.$(formSubmitSel))) {
      note('form', 'Submit button missing.', { formSubmitSel });
      return { ok: false, step: 'locate-submit', reason: 'Submit button not found', actions };
    }
    note('form', 'Submitting form.', { formSubmitSel });

    const postRespPromise = page.waitForResponse(
      (resp) => resp.request().method() === 'POST' && /\/parties\/1\/treasury(?!\S)/.test(resp.url()),
      { timeout: NAV_TIMEOUT }
    ).catch(() => null);

    await Promise.all([
      page.waitForNavigation({ waitUntil: 'networkidle2', timeout: NAV_TIMEOUT }).catch(() => null),
      page.click(formSubmitSel),
    ]);
    ...
    // Quick verification pass; if nothing obvious, try one safe re-submit
    ...
    if (!postResp && !verifiedSubmit) {
      note('retry', 'No POST/verification detected. Retrying submit click once.');
      await Promise.all([
        page.waitForNavigation({ waitUntil: 'networkidle2', timeout: NAV_TIMEOUT }).catch(() => null),
        page.click(formSubmitSel)
      ]);
    ...
    note('form', 'Form submitted.', { currentUrl: page.url() });
    ...
    if (formIssues && (formIssues.invalids.length || formIssues.alerts.length)) {
      ...
      // If the site surfaced an error, fail fast with the exact message
